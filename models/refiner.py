    ...
    def __init__(self, hparams):
        ...
        self.generator = PPGNGenerator(alpha=0.2, n_max=args.n_max, noise_latent_dim=args.generator_noise_latent_dim, n_layers=args.generator_layers, data_channels=args.generator_data_channels, 
                                  gelu=True, k_eigval=args.k, use_fixed_emb=False, normalization='instance',
                                    ...
                                    no_cond=False, init_emb_channels=args.generator_init_emb_channels, qm9=False,
        ...
        self.discriminator = PPGNDiscriminator(alpha=0.2, n_max=args.n_max, n_layers=args.discriminator_layers, data_channels=args.discriminator_data_channels,
                                        ...
                                        qm9=False, data_channels_mult=1)


        self.criterion = nn.BCEWithLogitsLoss()
        
        self.automatic_optimization = False
        self.train_dicriminator = True 
...
#         scheduler = self.lr_schedulers()
        ...
        noisy_real_eigval = batch[1]#.to(device)
        noisy_real_eigvec = batch[0]#.to(device)
        ...
        noisy_real_eigval += torch.randn_like(noisy_real_eigval)*1e-1
        noisy_real_eigvec += torch.randn_like(noisy_real_eigvec)*3e-2        
        noisy_real_eigvec[torch.logical_not(mask_real)] = 0
        
        num_gt = noisy_real_eigval.shape[0]
        noisy_gen_eigval = torch.cat([noisy_real_eigval, gen[1][:,0]],0)
        noisy_gen_eigvec = torch.cat([noisy_real_eigvec, gen[0]],0)
        ...
        if self.train_dicriminator:
            ...
            tot_dis_loss=fake_loss+true_loss
...
#         test_set.get_extra_data(True)
        ...
        xx = xx.to(device)
        yy = yy[:,0].to(device)
        mask = xx.abs().sum(-1)>1e-5
#         xx,yy = train_set.unscale_xy(xx,yy)

        with torch.no_grad():
            inno =  torch.randn(list(mask.shape[:2])+[self.generator.latent_dim], device=device)*1e-2
            fake_adj = self.generator(inno, yy, xx, mask)
            score = -self.discriminator( yy, xx, mask, fake_adj).cpu()
