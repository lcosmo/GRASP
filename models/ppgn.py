...
from .model_helper import masked_instance_norm2D, masked_layer_norm2D
...
class PowerfulLayer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        mask: batch x N x N x 1 --- B x N x 1
        out: batch x N x N x out_feat
        """ 
        norm = mask.squeeze(-1).float().sum(-1).sqrt().view(mask.size(0), 1, 1, 1)
#         mask = mask.unsqueeze(1).squeeze(-1) # B x 1 x N x N
        mask = mask[:,:,0] # B x N
#         out1 =  mask[:,None,None,:] * self.m1(x).permute(0, 3, 1, 2) * mask[:,None,:,None]           # batch, out_feat, N, N
#         out2 = mask[:,None,None,:] * self.m2(x).permute(0, 3, 1, 2) * mask[:,None,:,None]           # batch, out_feat, N, N
        out1 =  self.m1(x).permute(0, 3, 1, 2) * mask[:,None,:,None]           # batch, out_feat, N, N
        out2 =  self.m2(x).permute(0, 3, 1, 2) * mask[:,None,:,None]           # batch, out_feat, N, N
